...
enum {
    ARGV_CMD, ARGV_TARGET_IP, ARGV_SPOOF_IP, ARGV_SPOOF_MAC
};
...
int main(int argc, char *argv[])
{
    ...
    make_arp_header(&(arp_packet.ether_arp), \
                    arp_source_mac,arp_source_ip,NULL,arp_target_ip,ARPOP_REQUEST);

    while(1){
        pcap_sendpacket(adhandle,(unsigned char *)&arp_packet,\
                        sizeof(arp_packet));
